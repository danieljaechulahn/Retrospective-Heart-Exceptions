...
stathist_thor2025q2 = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/stathist_thor2025q2.sas7bdat", NULL)

cand_thor2025q2 = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/cand_thor2025q2.sas7bdat", NULL)

ptr_hr_20180101_20181231_pub = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/ptr_hr_20180101_20181231_pub.sas7bdat", NULL)

ptr_hr_20190101_20191231_pub = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/ptr_hr_20190101_20191231_pub.sas7bdat", NULL)

ptr_hr_20200101_20201231_pub = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/ptr_hr_20200101_20201231_pub.sas7bdat", NULL)

ptr_hr_20210101_20211231_pub = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/ptr_hr_20210101_20211231_pub.sas7bdat", NULL)

ptr_hr_20220101_20221231_pub = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/ptr_hr_20220101_20221231_pub.sas7bdat", NULL)

ptr_hr_20230101_20231231_pub = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/ptr_hr_20230101_20231231_pub.sas7bdat", NULL)

ptr_hr_20240101_20241231_pub = read_sas("~/Desktop/R Project Data/HRStatJust2509DanielAhn/ptr_hr_20240101_20241231_pub.sas7bdat", NULL)
...
cand_variables = cand_thor2025q2 %>% select(PERS_ID, PX_ID, REC_TX_DT, CAN_REM_DT, CAN_REM_CD, CAN_DEATH_DT, PERS_OPTN_DEATH_DT, PERS_SSA_DEATH_DT, CAN_DEATH_DT, CAN_LISTING_CTR_CD, WL_ORG, CAN_LISTING_DT, CAN_AGE_IN_MONTHS_AT_LISTING) %>%
  mutate(death_date = pmin(PERS_OPTN_DEATH_DT, PERS_SSA_DEATH_DT, CAN_DEATH_DT, na.rm = TRUE))

cand_list = cand_variables %>% 
  filter(WL_ORG == "HR") %>%
  filter(CAN_LISTING_DT >= mdy("03-22-2016") & CAN_LISTING_DT <= mdy("05-31-2025")) %>% 
  filter(CAN_AGE_IN_MONTHS_AT_LISTING / 12 < 18) %>% left_join(stathist_thor2025q2)
...
```{r general plots regarding per center and per region approval and submission of exceptions}
...
cand_list = cand_variables %>% 
  filter(WL_ORG != "LU") %>%
  filter(CAN_LISTING_DT >= mdy("10-18-2018") & CAN_LISTING_DT <= mdy("05-31-2025")) %>% 
...
#This looks at the time that it takes from patients submitting the exceptions to the RRBs actually receiving the application forms.
cand_variables = cand_thor2025q2 %>% select(PERS_ID, PX_ID, REC_TX_DT, CAN_REM_DT, CAN_REM_CD, CAN_DEATH_DT, PERS_OPTN_DEATH_DT, PERS_SSA_DEATH_DT, CAN_DEATH_DT, CAN_LISTING_CTR_CD, WL_ORG, CAN_LISTING_DT, CAN_AGE_IN_MONTHS_AT_LISTING) %>%
  mutate(death_date = pmin(PERS_OPTN_DEATH_DT, PERS_SSA_DEATH_DT, CAN_DEATH_DT, na.rm = TRUE))
...
cand_list_status1 = cand_variables %>% 
  filter(CAN_LISTING_DT >= mdy("10-18-2018") & CAN_LISTING_DT <= mdy("05-31-2025")) %>% filter(CAN_AGE_IN_MONTHS_AT_LISTING / 12 >= 18) %>%
  left_join(just_form_hr, by = "PX_ID") %>% group_by(PX_ID) %>% arrange(FormEffectiveDt) %>%
  #filter(FormStatus_descrip == "Approved" | FormStatus_descrip == "Not Approved") %>% 
...
cand_list_all_statuses <- rbind(cand_list_status1, cand_list_status2, cand_list_status3, cand_list_status4) %>% 
  select(PX_ID, listing_description, time_from_status_to_review, REGION, FormStatus_descrip) %>%
  mutate(
...
#This code chunk isolates all candidates who have submitted at least one exception application that was denied 

cand_variables = cand_thor2025q2 %>% select(PERS_ID, PX_ID, REC_TX_DT, CAN_REM_DT, CAN_REM_CD, CAN_LAST_ACT_STAT_DT, CAN_LAST_INACT_STAT_DT,
                                        CAN_DEATH_DT, PERS_OPTN_DEATH_DT, PERS_SSA_DEATH_DT, CAN_DEATH_DT, CAN_LISTING_CTR_CD, WL_ORG, 
                                        CAN_LISTING_DT, CAN_AGE_IN_MONTHS_AT_LISTING) %>%
  mutate(death_date = pmin(PERS_OPTN_DEATH_DT, PERS_SSA_DEATH_DT, CAN_DEATH_DT, na.rm = TRUE),
         last_date = pmax(CAN_LAST_ACT_STAT_DT, CAN_LAST_INACT_STAT_DT, na.rm = TRUE))

notapproved1 = cand_variables %>% 
  filter(CAN_LISTING_DT >= mdy("10-18-2018") & CAN_LISTING_DT <= mdy("05-31-2025")) %>% filter(CAN_AGE_IN_MONTHS_AT_LISTING / 12 >= 18) %>%
  left_join(just_form_hr, by = "PX_ID") %>% group_by(PX_ID) %>% arrange(FormEffectiveDt) %>%
  filter(FormStatus_descrip == "Approved" | FormStatus_descrip == "Not Approved") %>% 
  filter(Exception == 1) %>% filter(any(FormStatus_descrip == "Not Approved")) %>% select(PX_ID, REC_TX_DT, CAN_REM_DT, death_date, last_date, CAN_REM_CD, FormEffectiveDt, FormExpirationDt, FormReceiptDt, FormStatus_descrip, ApplicationStatus_descrip, listing_description) %>%
...
notapproved3 = cand_variables %>% 
  filter(CAN_LISTING_DT >= mdy("10-18-2018") & CAN_LISTING_DT <= mdy("03-31-2025")) %>% filter(CAN_AGE_IN_MONTHS_AT_LISTING / 12 >= 18) %>%
  left_join(just_form_hr, by = "PX_ID") %>% group_by(PX_ID) %>% arrange(FormEffectiveDt) %>%
  filter(FormStatus_descrip == "Approved" | FormStatus_descrip == "Not Approved") %>% 
  filter(Exception == 1) %>% filter(any(FormStatus_descrip == "Not Approved")) %>% select(PX_ID, REC_TX_DT, CAN_REM_DT, death_date, last_date, CAN_REM_CD, FormEffectiveDt, FormExpirationDt, FormReceiptDt, FormStatus_descrip, ApplicationStatus_descrip, listing_description) %>%
  filter(any(grepl("Appeal", ApplicationStatus_descrip, ignore.case = TRUE)) | any(grepl("Refer", ApplicationStatus_descrip, ignore.case = TRUE))) %>%
...
notapproved3.1.1 <- notapproved3.1 %>% filter(ConditionMet == TRUE) %>%
  mutate(
    time_from_status_to_receipt = as.numeric(difftime(FormReceiptDt, lag(FormEffectiveDt), units = "days")),
    FormEffectiveDt = first(FormEffectiveDt)) %>% slice(n())

notapproved3.1.2 <- notapproved3.1 %>% filter(ConditionMet == FALSE) %>% slice(n())
...
notapproved3.2.1 <- notapproved3.2 %>% filter(all(ConditionMet == TRUE)) %>% arrange(FormEffectiveDt) %>%
  mutate(time_from_status_to_receipt = as.numeric(difftime(FormReceiptDt, first(FormEffectiveDt), units = "days")),
         FormEffectiveDt = first(FormEffectiveDt)) %>% slice(n())

notapproved3.2.2 <- notapproved3.2 %>% filter(any(ConditionMet == FALSE)) %>% arrange(FormEffectiveDt) %>%
  filter(row_number() > 1) %>% 
  mutate(time_from_status_to_receipt = as.numeric(difftime(FormReceiptDt, first(FormEffectiveDt), units = "days")),
         FormEffectiveDt = first(FormEffectiveDt)) %>% slice(n())
...
notapproved <- rbind(notapproved1, notapproved2, notapproved3) %>% select(-ConditionMet) %>% ungroup() %>% 
  group_by(PX_ID) %>% arrange(PX_ID, FormEffectiveDt) %>%
  mutate(
    time_from_status_to_receipt = case_when(
      is.na(time_from_status_to_receipt) ~ as.numeric(difftime(FormReceiptDt, FormEffectiveDt, units = "days")),
      TRUE ~ time_from_status_to_receipt))

triedappeal = notapproved %>% filter(any(grepl("Appeal", ApplicationStatus_descrip))) %>% slice(n())
...
notapproved_dataset <- notapproved %>% slice(n()) %>% 
  mutate(tx = ifelse(REC_TX_DT >= FormEffectiveDt & REC_TX_DT <= FormReceiptDt, 1, 0),
         death_det = case_when(
           ...
           TRUE ~ 0),
         receipt = ifelse(death_det == 0 & tx == 0 & CAN_REM_DT >= FormReceiptDt, 1, 0)) %>% 
  arrange(tx) %>% slice(n()) %>% 
  mutate(
    outcome = case_when(
      death_det == 1 ~ 1,
    ...
    time_from_status_to_removal = as.numeric(difftime(last_date, FormEffectiveDt, units = "days")),
    time = case_when(
      outcome == 1 ~ time_from_status_to_death_det,
      outcome == 2 ~ time_from_status_to_tx,
      outcome == 3 ~ time_from_status_to_receipt,
      TRUE ~ time_from_status_to_removal)) %>%
  filter(time >= 0) #removes errors in which patients have justification forms submitted after their registrations already ended.

totaltime <- sum(notapproved_dataset$time_from_status_to_receipt)
...
#this code chunk isolates all candidates who submitted exception applications that were all approved.
...
approved = cand_variables %>% 
  ...
  left_join(just_form_hr, by = "PX_ID") %>% group_by(PX_ID) %>% arrange(FormEffectiveDt) %>%
  filter(FormStatus_descrip == "Approved" | FormStatus_descrip == "Not Approved") %>% 
  filter(Exception == 1) %>% filter(all(FormStatus_descrip == "Approved")) %>% select(PX_ID, REC_TX_DT, CAN_REM_DT, death_date, last_date, CAN_REM_CD, FormEffectiveDt, FormExpirationDt, FormReceiptDt, FormStatus_descrip, ApplicationStatus_descrip, listing_description) %>%
  mutate(
    time_from_status_to_receipt = as.numeric(difftime(FormReceiptDt, FormEffectiveDt, units = "days"))) %>% 
  filter(!is.na(time_from_status_to_receipt) & time_from_status_to_receipt >= 0) %>%
  filter(FormEffectiveDt <= CAN_REM_DT)

approved1 = approved %>% group_by(PX_ID) %>% arrange(PX_ID, FormEffectiveDt) %>% 
  filter(any(grepl("Appeal", ApplicationStatus_descrip))) %>% slice(n())
...
totaltimeapproved <- sum(approved_dataset$time_from_status_to_receipt)
#SENSITIVITY ANALYSIS

```

``` {r creating cumulative incidence plots}

#combine the two groups (both approved and denied). Here we estimated the cumulative incidence of transplant before form receipt treating death or deterioration and form receipt by RRB as competing events.
...
total$outcome <- factor(total$outcome,
                        ...
                        labels = c("Censored", "Death or Deterioration\nBefore Form Receipt", "Transplant Before\nForm Receipt", "Form Receipt by RRB"))
...
cif_plot <- survfit2(Surv(time, outcome) ~ FormStatus_descrip, data = total) %>%
  ...
  add_risktable(size = 6, theme = theme_risktable_default(plot.title.size = 16, axis.text.y.size = 16)) +
  ...
  xlab("Days after Submitting Status Exception Application") +
  theme_classic() +
  #guides(linetype=guide_legend(nrow = 1, byrow=TRUE)) +
  #guides(color = guide_legend(nrow = 1, byrow=TRUE)) +
  scale_color_manual(values = c('#374e55', '#df8f44', '#00a1d5')) +
...
cif_plot <- survfit2(Surv(time, outcome) ~ FormStatus_descrip, data = total) %>%
  ...
  xlab("Days after Submitting Status Exception Application") +
  theme_classic() +
  scale_color_manual(name = "Outcome", values = c('#374e55', '#df8f44', '#00a1d5')) +
  scale_fill_manual(name = "Outcome", values = c('#374e55', '#df8f44', '#00a1d5')) +
  scale_linetype_discrete(name = "RRB Decision", labels = c("Approved", "Denied")) +
  guides(
    fill = guide_legend(title = "Outcome", nrow = 1, order = 1),
    color = guide_legend(title = "Outcome", nrow = 1, order = 1),
    linetype = guide_legend(title = "RRB Decision", nrow = 1, order = 2)
  ) +
  theme(plot.title = element_text(size = 18),
  ...
  annotate("text", x = 0, y = 0.81, hjust = 0, size = 5,
           label = paste0(
             "Form Receipt by RRB p: ", "0.06"))
#For the annotations above, I just copied the Fine-Gray results below.

#linetype = guide_legend(override.aes = list(fill = c(NA, NA)))
...
cifresults_receipt <- cuminc(Surv(time, outcome) ~ FormStatus_descrip, data = as.data.frame(total)) %>%
  tbl_cuminc(
    times = 14,
    outcomes = c("Death or Deterioration\nBefore Form Receipt", "Transplant Before\nForm Receipt", "Form Receipt by RRB"),
    estimate_fun = gtsummary::label_style_number(scale = 100, digits = 1))


just_transplants = total %>% filter(outcome == "Transplant Before\nForm Receipt") %>%
  mutate(
    timing_of_transplants = case_when(
      REC_TX_DT >= mdy("10-18-2018") & REC_TX_DT <= mdy("12-31-2018") ~ "Oct-Dec 2018",
      REC_TX_DT >= mdy("01-01-2019") & REC_TX_DT <= mdy("03-31-2019") ~ "Jan-Mar 2019",
      REC_TX_DT >= mdy("04-01-2019") & REC_TX_DT <= mdy("06-30-2019") ~ "Apr-Jun 2019",
      REC_TX_DT >= mdy("07-01-2019") & REC_TX_DT <= mdy("09-30-2019") ~ "Jul-Sep 2019",
      REC_TX_DT >= mdy("10-01-2019") & REC_TX_DT <= mdy("12-31-2019") ~ "Oct-Dec 2019",
      REC_TX_DT >= mdy("01-01-2020") & REC_TX_DT <= mdy("03-31-2020") ~ "Jan-Mar 2020",
      REC_TX_DT >= mdy("04-01-2020") & REC_TX_DT <= mdy("06-30-2020") ~ "Apr-Jun 2020",
      REC_TX_DT >= mdy("07-01-2020") & REC_TX_DT <= mdy("09-30-2020") ~ "Jul-Sep 2020",
      REC_TX_DT >= mdy("10-01-2020") & REC_TX_DT <= mdy("12-31-2020") ~ "Oct-Dec 2020",
      REC_TX_DT >= mdy("01-01-2021") & REC_TX_DT <= mdy("03-31-2021") ~ "Jan-Mar 2021",
      REC_TX_DT >= mdy("04-01-2021") & REC_TX_DT <= mdy("06-30-2021") ~ "Apr-Jun 2021",
      REC_TX_DT >= mdy("07-01-2021") & REC_TX_DT <= mdy("09-30-2021") ~ "Jul-Sep 2021",
      REC_TX_DT >= mdy("10-01-2021") & REC_TX_DT <= mdy("12-31-2021") ~ "Oct-Dec 2021",
      REC_TX_DT >= mdy("01-01-2022") & REC_TX_DT <= mdy("03-31-2022") ~ "Jan-Mar 2022",
      REC_TX_DT >= mdy("04-01-2022") & REC_TX_DT <= mdy("06-30-2022") ~ "Apr-Jun 2022",
      REC_TX_DT >= mdy("07-01-2022") & REC_TX_DT <= mdy("09-30-2022") ~ "Jul-Sep 2022",
      REC_TX_DT >= mdy("10-01-2022") & REC_TX_DT <= mdy("12-31-2022") ~ "Oct-Dec 2022",
      REC_TX_DT >= mdy("01-01-2023") & REC_TX_DT <= mdy("03-31-2023") ~ "Jan-Mar 2023",
      REC_TX_DT >= mdy("04-01-2023") & REC_TX_DT <= mdy("06-30-2023") ~ "Apr-Jun 2023",
      REC_TX_DT >= mdy("07-01-2023") & REC_TX_DT <= mdy("09-30-2023") ~ "Jul-Sep 2023",
      REC_TX_DT >= mdy("10-01-2023") & REC_TX_DT <= mdy("12-31-2023") ~ "Oct-Dec 2023",
      REC_TX_DT >= mdy("01-01-2024") & REC_TX_DT <= mdy("03-31-2024") ~ "Jan-Mar 2024",
      REC_TX_DT >= mdy("04-01-2024") & REC_TX_DT <= mdy("06-30-2024") ~ "Apr-Jun 2024",
      REC_TX_DT >= mdy("07-01-2024") & REC_TX_DT <= mdy("09-30-2024") ~ "Jul-Sep 2024",
      REC_TX_DT >= mdy("10-01-2024") & REC_TX_DT <= mdy("12-31-2024") ~ "Oct-Dec 2024",
      REC_TX_DT >= mdy("01-01-2025") & REC_TX_DT <= mdy("03-31-2025") ~ "Jan-Mar 2025",
      REC_TX_DT >= mdy("04-01-2025") & REC_TX_DT <= mdy("06-30-2025") ~ "Apr-Jun 2025"),
    timing_of_transplants = factor(timing_of_transplants, levels = c("Oct-Dec 2018", "Jan-Mar 2019", "Apr-Jun 2019", "Jul-Sep 2019", "Oct-Dec 2019", "Jan-Mar 2020", "Apr-Jun 2020", "Jul-Sep 2020", "Oct-Dec 2020", "Jan-Mar 2021", "Apr-Jun 2021", "Jul-Sep 2021", "Oct-Dec 2021", "Jan-Mar 2022", "Apr-Jun 2022", "Jul-Sep 2022", "Oct-Dec 2022", "Jan-Mar 2023", "Apr-Jun 2023", "Jul-Sep 2023", "Oct-Dec 2023", "Jan-Mar 2024", "Apr-Jun 2024", "Jul-Sep 2024", "Oct-Dec 2024", "Jan-Mar 2025", "Apr-Jun 2025")))

transplant_trend_plot <- ggplot(just_transplants, aes(x = timing_of_transplants, fill = FormStatus_descrip)) +
  geom_bar(position = "stack") +
  labs(title = "",
       x = "",
       y = "Number of Transplant Recipients",
       fill = "Decision on Application\nfor Exception") +
  scale_fill_manual(values = c("#374e55", "#df8f44"), 
                    labels = c("Approved", "Denied")) +  # Add custom labels here
  theme_classic() +
  theme(plot.title = element_text(size = 18),
...
just_transplant_numbers = just_transplants %>% group_by(timing_of_transplants) %>% summarise(count = n())

mk_result <- mk.test(just_transplant_numbers$count)
print(mk_result)




transplant_trend_plot_without_fill <- ggplot(just_transplants, aes(x = timing_of_transplants)) +
  geom_bar(position = "stack", fill = "#374e55") +
  labs(title = "",
       x = "",
       y = "Number of Transplant Recipients") +
       #fill = "Decision on Application\nfor Exception") +
  #scale_fill_manual(values=c("#374e55")) +
  theme_classic() +
  theme(plot.title = element_text(size = 18),
        axis.text.x = element_text(angle = 45, hjust = 1, size = 18),
        axis.title.x = element_text(size = 20),
...
#Here we make a table 1 

fortable1 <- total %>% select(PX_ID, FormStatus_descrip, listing_description) %>% left_join(cand_thor2025q2 %>% select(PX_ID, CAN_REM_CD, CAN_INIT_ACT_STAT_CD, CAN_RACE, CAN_ETHNICITY_SRTR, CAN_GENDER, CAN_ABO, CAN_DGN, CAN_AGE_AT_LISTING, CAN_AGE_IN_MONTHS_AT_LISTING, CAN_BMI, CAN_PRIMARY_PAY, CAN_FUNCTN_STAT, CAN_PCW_MEAN, CAN_HGT_CM, CAN_WGT_KG, CAN_CARDIAC_OUTPUT, CAN_LISTING_CTR_CD, CAN_IABP, CAN_ECMO, CAN_IV_INOTROP, CAN_VAD1, CAN_VAD2, CAN_VAD_TY, CAN_LISTING_DT)) %>% left_join(thor_support_device %>% select(PX_ID, OtherSpecify)) %>% group_by(PX_ID) %>%
  mutate(
    ...
    requested_status = case_when(
      grepl("Status 1", listing_description) ~ "Status 1",
      grepl("Status 2", listing_description) ~ "Status 2",
      grepl("Status 3", listing_description) ~ "Status 3",
      grepl("Status 4", listing_description) ~ "Status 4"),
    requested_status = factor(requested_status, levels = c("Status 1", "Status 2", "Status 3", "Status 4")),
    race = case_when(
    ...
    diagnosis = case_when(
      ...
      CAN_DGN >= 1100 & CAN_DGN < 1200 ~ "Re-Heart Transplant",
      TRUE ~ "Other"),
    ...
    MCS = case_when(
      ...
      MCS == "Other" & CAN_VAD_TY == 2 ~ "LVAD Only",
      MCS == "Other" & CAN_VAD_TY == 3 ~ "RVAD Only",
      TRUE ~ MCS),
    MCS = ifelse(MCS == "Other", NA, MCS),
    MCS = factor(MCS, levels = c("ECMO", "TAH", "IABP", "LVAD and RVAD", "LVAD Only", "RVAD Only"))) %>% slice(1) 
  
library(kidney.epi)  

fortable1 = fortable1 %>% left_join(risk_strat_data_hr %>% select(PX_ID, HrSevFailSodium, HrSevFailSodiumDt, HrSevFailCreatinine, HrSevFailCreatinineDt, HrSevFailBun, HrSevFailBunDt, HrSevFailAlbumin, HrSevFailAlbuminDt, HrSevFailBilirubin, HrSevFailBilirubinDt, HrSevFailBnp, HrSevFailBnpDt, HrSevFailNtBnpType)) %>% 
  mutate(lab_date = pmin(HrSevFailSodiumDt, HrSevFailCreatinineDt, HrSevFailBunDt, HrSevFailAlbuminDt, HrSevFailBilirubinDt, HrSevFailBnpDt, na.rm = TRUE)) %>% select(-HrSevFailSodiumDt, -HrSevFailCreatinineDt, -HrSevFailBunDt, -HrSevFailAlbuminDt, -HrSevFailBilirubinDt, -HrSevFailBnpDt) %>% group_by(PX_ID) %>% arrange(PX_ID, lab_date) %>% slice(1) %>%
  mutate(
    BNP = ifelse(HrSevFailNtBnpType == "BNP", 1, 0),
    proBNP = ifelse(HrSevFailNtBnpType == "NT Pro BNP", 1, 0),
    gender = ifelse(CAN_GENDER == "M", 1, 0),
    eGFR = egfr.ckdepi.cr.2021(creatinine = HrSevFailCreatinine, age = CAN_AGE_IN_MONTHS_AT_LISTING / 12, sex = gender, creatinine_units = "mg/dl"),
    stmcs = ifelse(MCS == "ECMO", 1, 0),
    stmcs = ifelse(is.na(stmcs), 0, stmcs),
    dlvad = ifelse(MCS == "LVAD Only", 1, 0),
    dlvad = ifelse(is.na(dlvad), 0, dlvad)
  )


# equation for calculating the uscrs
# short MCS ever defined as whether the candidate is currenty or has previously been supported by ECMO, temporary surgical lvad, or BiVAD
# durable LVAD as one of these device brands: Arrow Lionheart, Heartmate II, Heartmate IP, Heartmate VE, Heartmate XVE, Heartsaver VAD, Jarvik 2000, MicroMed DeBakey, Novacor PC, Novacor PCq,  Evaheart, Heartware HVAD, Worldheart Levacor, Heartmate III, ReliantHeartAssist 5, ReliantHeart aVAD.
#BNP_BNT_Pro is an indicator variable for BNP NT Pro. bnp_reg_indicator indicates the opposite (regular BNP)

fortable1 = fortable1 %>%
  #indicator for regular BNP
  mutate(
    uscrs_raw_score = 1.02*stmcs + 0.55*log(HrSevFailBilirubin + 1) - 0.001*eGFR + 0.40*log(HrSevFailBnp)*BNP + 0.20*log(HrSevFailBnp)*proBNP - 0.63*HrSevFailAlbumin - 0.07*HrSevFailSodium - 1.12*dlvad) 

# create breakpoints and labels from mapping_table
breaks <- sort(unique(c(uscrs_mapping_table$min_raw_score, max(uscrs_mapping_table$max_raw_score))))
labels <- uscrs_mapping_table$us_crs

# assign scores 1-50 based on mapping table
fortable1 <- fortable1 %>%
...
options(gtsummary.fexact.workspace = "2e+10")



fortable1 %>% ungroup() %>%
  dplyr::select(
    age_at_listing, sex, race, bmi, blood_type, diagnosis, requested_status, pcwp, cardiac_index, uscrs_score, MCS, FormStatus_descrip) %>%
...
justreceipts = total %>% filter(outcome == "Form Receipt by RRB")

data_vector = justreceipts$time
quantile(data_vector, probs = c(0.25, 0.5, 1))
...
notapproved_sensitivity <- notapproved %>% slice(n()) %>% 
  mutate(
    ...
    death_det = case_when(
      ...
      TRUE ~ 0),
    review = ifelse(death_det == 0 & tx == 0 & CAN_REM_DT >= extra_date, 1, 0)) %>% 
  arrange(tx) %>% slice(n()) %>% 
  mutate(
    outcome = case_when(
      death_det == 1 ~ 1,
      ...
      review == 1 ~ 3,
      TRUE ~ 0),
    time_from_status_to_review = as.numeric(difftime(extra_date, FormEffectiveDt, units = "days")),
    time_from_status_to_tx = ifelse(tx == 1, as.numeric(difftime(REC_TX_DT, FormEffectiveDt, units = "days")), NA),
    time_from_status_to_death_det = ifelse(death_det == 1, as.numeric(difftime(CAN_REM_DT, FormEffectiveDt, units = "days")), NA),
    time_from_status_to_removal = as.numeric(difftime(last_date, FormEffectiveDt, units = "days")),
    time = case_when(
      outcome == 1 ~ time_from_status_to_death_det,
      outcome == 2 ~ time_from_status_to_tx,
      outcome == 3 ~ time_from_status_to_review,
      TRUE ~ time_from_status_to_removal))
...
total_sensitivity$outcome <- factor(total_sensitivity$outcome,
                                    levels = c("0", "1", "2", "3"), # Assuming 0 is censored, adjust if different
                                    labels = c("Censored", "Death or Deterioration Before\nEstimated Form Review", 
                                               "Transplant Before\nEstimated Form Review", "Estimated RRB Review"))
...
cif_plot_sensitivity <- survfit2(Surv(time, outcome) ~ FormStatus_descrip, data = total_sensitivity) %>%
  ggcuminc(
    outcome = c("Death or Deterioration Before\nEstimated Form Review", "Transplant Before\nEstimated Form Review", "Estimated RRB Review"),
    linewidth = 1) + 
  add_confidence_interval(alpha = 0.5) +
...
cifresults_review <- cuminc(Surv(time, outcome) ~ FormStatus_descrip, data = as.data.frame(total_sensitivity)) %>%
  tbl_cuminc(
    times = 14,
    outcomes = c("Death or Deterioration Before\nEstimated Form Review", "Transplant Before\nEstimated Form Review", "Estimated RRB Review"),
    estimate_fun = gtsummary::label_style_number(scale = 100, digits = 1))


cif_plots_combined <- plot_grid(cif_plot, cif_plot_sensitivity, ncol = 1, nrow = 2, labels = c("A", "B"), label_size = 20)
...
``` {r transplants among denied candidates}
#isolate the patients who received a transplant with exceptions that were ultimately denied

exception_after_listing <- notapproved_dataset %>% filter(tx == 1) %>% left_join(cand_thor2025q2 %>% select(PX_ID, CAN_LISTING_DT)) %>%
  left_join(stathist_thor2025q2 %>% select(PX_ID, CANHX_BEGIN_DT, CANHX_END_DT, CANHX_STAT_CD), by = "PX_ID") %>% 
...
prev_status <- exception_after_listing %>%
  ...
  mutate(
    ...
    status_before_exception = case_when(
      CANHX_STAT_CD == 2110 | CANHX_STAT_CD == 1110 ~ "1",
      CANHX_STAT_CD == 2120 | CANHX_STAT_CD == 1120 ~ "2",
      CANHX_STAT_CD == 2130 | CANHX_STAT_CD == 1130 ~ "3",
      CANHX_STAT_CD == 2140 | CANHX_STAT_CD == 1140 ~ "4",
      CANHX_STAT_CD == 2150 | CANHX_STAT_CD == 1150 ~ "5",
      CANHX_STAT_CD == 2160 | CANHX_STAT_CD == 1160 ~ "6",
...
denied_exception_tx <- notapproved_dataset %>% filter(tx == 1) %>% left_join(prev_status) %>%
  left_join(stathist_thor2025q2 %>% select(PX_ID, CANHX_BEGIN_DT, CANHX_END_DT, CANHX_STAT_CD), by = "PX_ID") %>% arrange(CANHX_BEGIN_DT) %>% slice(n()) %>%
  mutate(
    actual_status = case_when(
      ...
      CANHX_STAT_CD == 2140 | CANHX_STAT_CD == 1140 ~ "4",
      CANHX_STAT_CD == 2150 | CANHX_STAT_CD == 1150 ~ "5",
      CANHX_STAT_CD == 2160 | CANHX_STAT_CD == 1160 ~ "6")) %>% left_join(cand_thor2025q2 %>% select(PX_ID, DONOR_ID))
...
no_status_before_exception <- no_status_before_exception %>% 
  left_join(cand_thor2025q2 %>% select(PX_ID, CAN_LISTING_DT, CAN_IABP, CAN_ECMO, DONOR_ID, CAN_DGN, CAN_VAD1, CAN_VAD2, CAN_VAD_TAH, CAN_VAD_TY, CAN_PREV_HR), by = "PX_ID") %>%
  mutate(
    diagnosis = case_when(
      CAN_DGN > 1048 & CAN_DGN < 1100 ~ "Restrictive Cardiomyopathy",
      CAN_DGN == 1201 ~ "Hypertrophic Cardiomyopathy",
...
#Here we estimated what statuses the 115 candidates with denied exceptions who received transplants should have had if they didn't have those exceptions. We used hemodynamic and other MCS data from the status justification files.
...
figure_for_denied_transplants <- plot_grid(plot_actual_status, plot_predicted_status, ncol = 1, nrow = 2, labels = c("A", "B"), label_size = 18)
...
data_for_match_run <- denied_exception_tx %>% left_join(tx_hr2025q2 %>% select(PX_ID, REC_TX_ORG_TY, DON_AGE)) %>% ungroup()
...
#this calculates how many donors are in the match run data up to end of 2024
included_donors = full_match_runs %>% group_by(DONOR_ID) %>% slice(1)
...
data_for_match <- full_match_runs %>% select(-PTR_ROW_ORDER, -PTR_SEQUENCE_NUM, -PTR_STAT_CD) %>% ungroup() %>% filter(PX_ID %in% c(recipient_ids) & DONOR_ID %in% c(donor_ids)) %>% left_join(data_for_match_run %>% select(PX_ID, REC_TX_DT, actual_status, predicted_status_without_exception)) %>% 
  left_join(cand_thor2025q2 %>% select(PX_ID, CAN_ABO), by = "PX_ID") %>% 
  left_join(donor_deceased2025q2 %>% select(DONOR_ID, DON_ABO), by = "DONOR_ID") %>%
  mutate(
...
skipped_candidates = revised_match_runs %>% ungroup() %>% select(PX_ID.x, MATCH_SUBMIT_DT) %>% rename(PX_ID = PX_ID.x) %>% 
  ...
  left_join(cand_thor2025q2 %>% select(PX_ID, CAN_LISTING_DT, CAN_REM_DT, CAN_REM_CD, CAN_LAST_ACT_STAT_DT)) %>%
  mutate(
    OUTCOME = case_when(
      CAN_REM_CD == 4 ~ 1,
...
``` {r missing data for 174 patients}

library(kidney.epi)

# Get the 174 denied patients who transplanted
denied_tx_uscrs <- notapproved_dataset %>%
  filter(tx == 1) %>%
  select(PX_ID, FormStatus_descrip, listing_description) %>%
  left_join(cand_thor2025q2 %>% 
              select(PX_ID, CAN_GENDER, CAN_AGE_IN_MONTHS_AT_LISTING, CAN_VAD1, 
                     CAN_VAD_TY, CAN_ECMO, CAN_IABP), 
            by = "PX_ID") %>%
  left_join(thor_support_device %>% select(PX_ID, OtherSpecify), by = "PX_ID") %>%
  left_join(risk_strat_data_hr %>% 
              select(PX_ID, HrSevFailSodium, HrSevFailSodiumDt, HrSevFailCreatinine, 
                     HrSevFailCreatinineDt, HrSevFailBun, HrSevFailBunDt, 
                     HrSevFailAlbumin, HrSevFailAlbuminDt, HrSevFailBilirubin, 
                     HrSevFailBilirubinDt, HrSevFailBnp, HrSevFailBnpDt, 
                     HrSevFailNtBnpType), 
            by = "PX_ID") %>%
  mutate(lab_date = pmin(HrSevFailSodiumDt, HrSevFailCreatinineDt, HrSevFailBunDt, 
                         HrSevFailAlbuminDt, HrSevFailBilirubinDt, HrSevFailBnpDt, 
                         na.rm = TRUE)) %>%
  group_by(PX_ID) %>%
  arrange(PX_ID, lab_date) %>%
  slice(1) %>%
  ungroup()

# Determine MCS type using your exact logic
denied_tx_uscrs <- denied_tx_uscrs %>%
  mutate(
    MCS = case_when(
      any(grepl("impella", OtherSpecify, ignore.case = TRUE)) | 
        CAN_VAD1 %in% c(215, 225, 226, 237, 311, 317, 318, 331, 332) ~ "PEVAD",
      CAN_ECMO == 1 | any(OtherSpecify == "Cardiohelp") ~ "ECMO",
      CAN_IABP == 1 ~ "IABP",
      TRUE ~ "Other"),
    MCS = case_when(
...
denied_tx_uscrs <- denied_tx_uscrs %>%
  mutate(
    ...
    proBNP = ifelse(HrSevFailNtBnpType == "NT Pro BNP", 1, 0),
    gender = ifelse(CAN_GENDER == "M", 1, 0),
    eGFR = egfr.ckdepi.cr.2021(creatinine = HrSevFailCreatinine, 
                               age = CAN_AGE_IN_MONTHS_AT_LISTING / 12, 
                               sex = gender, 
    ...
    dlvad = ifelse(MCS == "LVAD Only", 1, 0),
    dlvad = ifelse(is.na(dlvad), 0, dlvad),
    uscrs_raw_score = 1.02*stmcs + 0.55*log(HrSevFailBilirubin + 1) - 0.001*eGFR + 
      0.40*log(HrSevFailBnp)*BNP + 0.20*log(HrSevFailBnp)*proBNP - 
      0.63*HrSevFailAlbumin - 0.07*HrSevFailSodium - 1.12*dlvad
  )

# Apply mapping table to get 1-50 score
breaks <- sort(unique(c(uscrs_mapping_table$min_raw_score, max(uscrs_mapping_table$max_raw_score))))

denied_tx_uscrs <- denied_tx_uscrs %>%
  mutate(uscrs_score = cut(uscrs_raw_score, breaks = breaks, labels = FALSE, include.lowest = TRUE))

# Get actual status at transplant (Status 1, 2, 3, or 4)
denied_tx_uscrs <- denied_tx_uscrs %>%
  mutate(actual_status_at_tx = case_when(
    grepl("Status 1", listing_description) ~ 1,
    grepl("Status 2", listing_description) ~ 2,
    grepl("Status 3", listing_description) ~ 3,
    grepl("Status 4", listing_description) ~ 4,
    TRUE ~ NA_real_
  ))

# Calculate median US-CRS by actual status at transplant
uscrs_by_status <- denied_tx_uscrs %>%
  filter(!is.na(uscrs_score) & !is.na(actual_status_at_tx)) %>%
  group_by(actual_status_at_tx) %>%
  summarise(
    n = n(),
    median_uscrs = median(uscrs_score, na.rm = TRUE),
    q25 = quantile(uscrs_score, 0.25, na.rm = TRUE),
    q75 = quantile(uscrs_score, 0.75, na.rm = TRUE)
  )

print(uscrs_by_status)

# Also check how many are missing US-CRS
missing_uscrs <- denied_tx_uscrs %>%
  summarise(
    total = n(),
    missing_uscrs = sum(is.na(uscrs_score)),
    pct_missing = 100 * missing_uscrs / total
  )
