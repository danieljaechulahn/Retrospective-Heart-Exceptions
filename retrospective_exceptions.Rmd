...
cand_variables = cand_thor24 %>% select(PERS_ID, PX_ID, REC_TX_DT, CAN_REM_DT, CAN_REM_CD, CAN_LAST_ACT_STAT_DT, CAN_LAST_INACT_STAT_DT,
...
totaltime <- sum(notapproved$time_from_status_to_receipt)
...
totaltimeapproved <- sum(approved$time_from_status_to_receipt)
...
quantile(data_vector, probs = c(0.25, 0.5, 0.99))
